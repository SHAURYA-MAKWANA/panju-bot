...
config: {
    name: "panju",
    description: "panju Menu",
    category: 'utility',
    usage: "1) !panju \n2) !panju [module name]\n3) !panju [command (name or alias)]",
    example: "1) !panju\n2) !panju util\n3) !panju ban",
    aliases: ['h']
},
...
if (!args[0]) return message.channel.send(Embed);
    ...
    const embed = new Discord.MessageEmbed()
    .setColor("RANDOM")
    .setAuthor(`${message.guild.me.displayName} panju`, message.guild.iconURL())
    .setThumbnail(bot.user.displayAvatarURL())

    let command = bot.commands.get(bot.aliases.get(args[0].toLowerCase()) || args[0].toLowerCase())
    if (!command) return message.channel.send(embed.setTitle("**Invalid Command!**").setDescription(`**Do \`${prefix}panju\` For the List Of the Commands!**`))
